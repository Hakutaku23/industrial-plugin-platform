  ...
  <main class="industrial-shell">
    <header class="topbar">
      ...
      <nav v-if="!showLoginPage" class="nav-menu">
        ...
        <RouterLink v-if="can('package.read')" to="/packages/upload">算法上传</RouterLink>
        <RouterLink v-if="can('package.read')" to="/packages">算法资产</RouterLink>
        <RouterLink v-if="can('datasource.read')" to="/data-sources">数据接入</RouterLink>
        <RouterLink v-if="can('instance.read')" to="/instances">实例编排</RouterLink>
        <RouterLink v-if="can('run.read')" to="/runs">运行监管</RouterLink>
        ...
        <RouterLink v-if="can('user.read')" to="/admin/users">权限管理</RouterLink>
      </nav>
...
.eyebrow { margin: 0; font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em; color: #94a3b8; font-weight: 600; }
