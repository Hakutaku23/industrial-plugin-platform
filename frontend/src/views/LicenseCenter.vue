    ...
    <div class="grid">
      <article class="card">
        ...
        <div v-if="status" class="kv-grid">
          ...
          <div><span>撤销</span><strong>{{ status.revoked ? 'Yes' : 'No' }}</strong></div>
          <div><span>授权类型</span><strong>{{ status.grant_mode || '-' }}</strong></div>
          <div><span>客户</span><strong>{{ status.customer_name || '-' }}</strong></div>
          <div><span>许可证 ID</span><strong class="mono">{{ status.license_id || '-' }}</strong></div>
          ...
          <div><span>宽限天数</span><strong>{{ status.grace_days ?? 0 }}</strong></div>
          <div><span>宽限截止</span><strong>{{ status.grace_expires_at || '-' }}</strong></div>
      ...
      <article class="card wide-card">
        <h2>撤销清单</h2>
        <div v-if="revocations" class="kv-grid">
          <div class="wide"><span>清单路径</span><strong class="mono">{{ revocations.path }}</strong></div>
          <div><span>撤销数量</span><strong>{{ revocations.revoked_license_ids?.length || 0 }}</strong></div>
          <div><span>更新时间</span><strong>{{ revocations.updated_at || '-' }}</strong></div>
          <div class="wide">
            <span>撤销许可证 ID</span>
            <strong class="mono block">{{ (revocations.revoked_license_ids || []).join('\n') || '-' }}</strong>
          </div>
        </div>
      </article>
